...
* Python source files:
  * The Python package source files are located in the `electricity_usage` directory.
  * The file `electricity_usage.__main__.py` defines a command line interface that
    is accessible both as the command `electricity_usage` and via
    `python -m electricity_usage`.
  * `tests/test_cli.py` contains rudimentary testing of this CLI.
  * `tests/test_electricity_usage.py` contains the unit tests for the package.
  * `tests/conftest.py` contains testing setup and configuration for `pytest`
  * The `notebooks` directory contains an example Jupyter notebook on how to use `electricity_usage`.
